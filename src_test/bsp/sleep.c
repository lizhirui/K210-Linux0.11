...
int pusleep(uint64_t usec)
{
    uint64_t cycle = core_get_cycle();
    uint64_t nop_all = usec * sysctl_clock_get_freq(SYSCTL_CLOCK_CPU) / 1000000UL;
    while(1)
    {
        if(core_get_cycle() - cycle >= nop_all)
            break;
    }
    return 0;
}

int pmsleep(uint64_t msec)
{
    return (unsigned int)pusleep(msec * 1000);
}

unsigned int psleep(unsigned int seconds)
{
    return (unsigned int)pmsleep(seconds * 1000);
}
