...
void plic_init(void)
{
    ...
    /* Get current core id */
    unsigned long core_id = core_get_coreid();
...
handle_irq_m_ext(uintptr_t cause, uintptr_t epc)
{
    ...
    {
        /* Get current core id */
        uint64_t core_id = core_get_coreid();
        ...
        trap_info.int_num = int_num;
        trap_info.int_threshold = int_threshold;
        syslog_debug("handle_irq_m_ex","claim_complete = %d,priority_threshold = %d",trap_info.int_num,trap_info.int_threshold);
